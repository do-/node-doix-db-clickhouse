...
*/
		...
		let {url} = this, {sql} = call; if (wantResult) {
			
			url += '&output_format_json_quote_decimals=1&output_format_decimal_trailing_zeros=1'

			sql += ' FORMAT JSONCompactEachRowWithNamesAndTypes'

		}
		else {

			url += '&max_partitions_per_insert_block=0'

		}

		const req = call.raw = http.request (url, {...this.pool.options, method: 'POST'})
