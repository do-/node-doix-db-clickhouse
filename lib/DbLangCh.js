...
const {DbLang, DbColumn, DbType, DbTypeArithmeticFloat, DbTypeArithmeticInt, DbTypeCharacter, DbTypeDate, DbTypeTimestamp, DbTypeArithmeticFixed, DbTable, DbView, DbFunction, DbProcedure} = require ('doix-db')
...
const TYPE_MAP = new Map ([ 
	...
	['Float32', DbTypeArithmeticFloat],
	['Float64', DbTypeArithmeticFloat],
...
class DbLangCh extends DbLang {
	...
	compareColumns (asIs, toBe) {
		...
		for (const k of ['codec', 'comment']) if (asIs [k] !== toBe [k]) diff.push (k)

		return diff

	}

	genCommentClause (comment) {

		if (!comment) return ''

		return ` COMMENT ${this.quoteLiteral (comment)}`

	}

	* genColumnConstraints ({codec, comment}) {

		if (comment) yield this.genCommentClause (comment)
	...
	getDbObjectClass (o) {
		...
		const c = super.getDbObjectClass (o)

		if (c === DbProcedure) return DbFunction
	...
	genCreateTable ({qName, pk, columns, engine, partitionBy, comment}) {
		...
		return sql + this.genCommentClause (comment)
	...
	* genMigrate (asIs, toBe) {
	...
	genReCreateView ({qName, options, specification, sql, comment}) {

		return [`CREATE OR REPLACE ${options} VIEW ${qName} ${specification} AS ${sql}${this.genCommentClause (comment)}`]

	}	
	
	genReCreateFunction ({qName, parameters, body}) {

		return [`CREATE OR REPLACE FUNCTION ${qName} AS (${parameters.map (i => i.qName)}) -> ${body}`]
